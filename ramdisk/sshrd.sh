...
if [ "$oscheck" = 'Darwin' ]; then
    ...
    if [ ! "$2" = 'rootless' ]; then
        ...
        unzip palera1n.zip -d work/palera1n
        unzip work/palera1n/palera1n.ipa -d work/palera1n/palera1n
        rm -rf /tmp/SSHRD/usr/local/bin/loader.app/*
        cp -R work/palera1n/palera1n/Payload/palera1nLoader.app/* /tmp/SSHRD/usr/local/bin/loader.app
    ...
    if [ ! "$2" = 'rootless' ]; then
        curl -LO curl -LO https://nightly.link/palera1n/loader/workflows/build/main/palera1n.zip
        mv palera1n.zip work/palera1n.zip
                unzip work/palera1n.zip -d work/palera1n.zip
        unzip work/palera1n/palera1n.ipa -d work/palera1n/palera1n
        mkdir -p work/palera1n/uwu/usr/local/bin/palera1nLoader.app
        cp -R work/palera1n/palera1n/Payload/palera1nLoader.app/* work/palera1n/uwu/usr/local/bin/palera1nLoader.app

        "$oscheck"/hfsplus work/ramdisk.dmg rmall usr/local/bin/palera1nLoader.app > /dev/null
        "$oscheck"/hfsplus work/ramdisk.dmg addall work/palera1n/uwu > /dev/null
        "$oscheck"/hfsplus work/ramdisk.dmg mv /usr/local/bin/palera1nLoader.app/palera1n /usr/local/bin/palera1nLoader.app/Tips > /dev/null
    fi
fi
